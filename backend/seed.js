...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/prism';
...
const seedDatabase = async () => {
  try {
    ...
    if (!creator) {
      console.log('🌱 Creating a default Creator user account...');
      ...
      console.log(`ℹ️ Found existing Creator user: ${creator.username}`);
    }

    // 2. Insert videos if none exist
    const videoCount = await Video.countDocuments();
    if (videoCount === 0) {
      console.log('🌱 Seeding videos list...');
      const videosWithCreator = SEED_VIDEOS.map(video => ({
        ...video,
        creator: creator._id,
        likes: [creator._id], // pre-like
      }));
      
      const inserted = await Video.insertMany(videosWithCreator);
      console.log(`✅ Successfully seeded ${inserted.length} videos into the database!`);
    } else {
      console.log(`ℹ️ Database already has ${videoCount} videos. Skipping video seeding.`);
    }
